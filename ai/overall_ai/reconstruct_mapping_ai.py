...
def compare_data(raw_data, computed_data, expected_matches: int):
    ...
    for index, i in enumerate(computed_data):
        test = [
            ...
            and (
                i["lecturer"] is None
                or all(a in e["content"] for a in i["lecturer"].split(", "))
            )
        ...
        if index == 44:
            ...
            print(
                f"Found: {[e for e in raw_data if i['module_code'] == e['module_code']]}"
            )
...
if __name__ == "__main__":
    result = read_data()
    result = compare_data(
        result["raw_data"], result["computed_data"], expected_matches=37517
    )
