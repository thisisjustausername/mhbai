...
Tools:
Rufe ValidateOutput().forward(data) von ValidateOutput mit data als ModuleHandbook Objekt (json str und dict sind auch erlaubt) auf, um die Ausgabe zu validieren. Nur wenn bei der Ausgabe unter valid True erscheint, darf final answer aufgerufen werden.
Falls valid True ist und die Ausgabe vollständig ist, MÜSSEN Sie mit denselben Daten direkt final_answer(data) von FinalAnswerTool() aufrufen, um die Ausgabe zurückzugeben. Das ist sehr wichtig.
ValidateOutput wird folgendermaßen importiert: from ai.information_extraction.tools import ValidateOutput
Um es zu verwenden, benutze: ValidateOutput().forward(data)

Absolute Regeln für Tools:
1. Rufe ValidateOutput().forward(data) von ValidateOutput mit data als ModuleHandbook Objekt (json str und dict sind auch erlaubt) auf, um die Ausgabe zu validieren. Nur wenn bei der Ausgabe unter valid True erscheint, darf final_answer(data) von FinalAnswerTool() aufgerufen werden.
2. Falls valid True ist, MÜSSEN Sie mit denselben Daten direkt final_answer(data) von FinalAnswerTool() aufrufen, um die Ausgabe zurückzugeben. Das ist sehr wichtig.
...
Tools:
FinalAnswerTool() von ai.information_extraction.tools FinalAnswerTool() wird zur finalen Ausgabe und Validierung der Ausgabe verwendet und gibt die finale Antwort zurück. Dieses Tool darf nur ein einziges Mal aufgerufen werden. Der Parameter 'answer' muss im gleichen Call angegeben werden. Nur, falls die Validierung fehlschläft, ist ein weiterer Aufruf erlaubt.
FinalAnswerTool() wird folgendermaßen importiert: from ai.information_extraction.tools import FinalAnswerTool
Um es zu verwenden, benutze: FinalAnswerTool().forward(answer)
FinalAnswerTool() erwartet als Parameter 'answer' ein ModuleHandbook Objekt. FinalAnswerTool() validiert die Ausgabe und gibt die finale Antwort zurück, falls die Validierung erfolgreich ist. Falls die Validierung fehlschlägt, wird ein ValueError ausgelöst.

FinalAnswerTool().forward(answer) MUSS verwendet werden, um die finale Antwort zurückzugeben.

Die Aufgabe gilt NUR als abgeschlossen, wenn FinalAnswerTool erfolgreich aufgerufen wurde.

Nach einem erfolgreichen Aufruf darf KEINE weitere Ausgabe erfolgen (kein Text, keine Erklärungen).

Falls die Validierung fehlschlägt:
- korrigiere die Daten
- rufe FinalAnswerTool erneut auf

Wiederhole dies, bis die Ausgabe erfolgreich ist.

Datenklassen:
Die Datenklassen ModuleHandbook, ModuleInfo,  können von ai.information_extraction.data_template importiert werden. Sie dienen dazu, strikte Typanforderungen für die KI zu setzen, wenn Informationen über Module, Prüfungen und Zeitaufwand extrahiert werden.
...
if __name__ == "__main__":
    ...
    agent = CodeAgent(
        ...
        additional_authorized_imports=["json", "pydantic", "typing", "ai.information_extraction.data_template"],
        model=model)
