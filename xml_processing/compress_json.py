...
def compress_json(local_path: Path) -> dict[str, Any] | None:
    ...
    for i in data.get("module_groups", []):
        ...
        for j in i.get("modules", []):
            ...
            langs = {e for e in [e.get("language", None) for e in j.get("languages", [])] if e is not None}
            ...
            for e in j.get("exams", []):
                ...
                langs = {a for a in [a.get("language", None) for a in e.get("languages", [])] if a is not None}
            ...
            for k in j.get("parts", []):
                ...
                langs = {a for a in [a.get("name", None) for a in k.get("languages", [])] if a is not None}
                part["languages"] = ", ".join(langs) if len(langs) > 0 else None
                tms = [l.get("name", None) for l in (k.get("teaching_methods", []) or []) if l is not None]
                tms = [l for l in tms if l is not None]
                part["teaching_methods"] = ", ".join(tms) if len(tms) > 0 else None
    ...
    recursive_walk(mhb, remove_keys = [], rename_keys = {}, retype_values={}, replace_values = {"keine Angabe": None, 'NA': None})
    return mhb
