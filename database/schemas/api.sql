...
CREATE TABLE IF NOT EXISTS api.sessions (
    ...
    user_id INTEGER REFERENCES api.users(id) ON DELETE CASCADE NOT NULL,
    session_id UUID NOT NULL UNIQUE
);

-- table to save configuration settings
CREATE TABLE IF NOT EXISTS api.configurations (
    id SERIAL PRIMARY KEY,
...
INSERT INTO api.configurations (key, value) VALUES
('session_expiration_days', '30'),
('reset_code_expiration_minutes', '15');

CREATE TABLE IF NOT EXISTS api.verification_codes (
    id SERIAL PRIMARY KEY,
...
CREATE TABLE IF NOT EXISTS error_logs (
    ...
    raised_by INTEGER REFERENCES api.error_logs(id), -- self reference for errors raised by error handling
